...
CLI_READY_DATA = bytes([
    0x00, 0x22, 0x00, 0x01, 0x01, 0x10, 0x16, 0x4f,
    0x00, 0x01, 0x00, 0x04, 0x01, 0x10, 0x16, 0x4f,
    0x00, 0x13, 0x00, 0x04, 0x01, 0x10, 0x16, 0x4f,
    0x00, 0x02, 0x00, 0x01, 0x01, 0x10, 0x16, 0x4f,
    0x00, 0x03, 0x00, 0x01, 0x01, 0x10, 0x16, 0x4f,
    0x00, 0x15, 0x00, 0x01, 0x01, 0x10, 0x16, 0x4f,
...
# Максимальная длина текста в байтах (cp1251) для ICQ-протокола.
# Поле длины TLV — 16-битное, но реальный лимит старых серверов ~7996 байт.
ICQ_MAX_MSG_BYTES = 1000

# ── Message sanitization ──────────────────────────────────────────────────────

# Символы cp1251, которые стоит заменить на читаемые ASCII-аналоги
...
def sanitize_for_icq(text: str, max_bytes: int = ICQ_MAX_MSG_BYTES) -> str:
    """
    Подготавливает текст к отправке через ICQ (cp1251):
    1. Заменяет известные Unicode-символы на cp1251-совместимые аналоги.
    2. Удаляет эмодзи и всё, что не кодируется в cp1251.
    3. Схлопывает лишние пробелы/переносы, оставшиеся после вырезания.
    4. Обрезает до max_bytes байт, не разрывая строки посередине символа.
    """
    if not text:
        return ""

    # Шаг 1: замена известных символов на аналоги
    for src, dst in _CP1251_TRANSLITERATION.items():
        text = text.replace(src, dst)

    # Шаг 2: убираем всё, что cp1251 не знает
    # Пробуем закодировать каждый символ; нечитаемое — выбрасываем
    cleaned_chars = []
    for ch in text:
        try:
            ch.encode("cp1251")
            ...
            # Для управляющих символов оставляем только \n, \r, \t
            if ch in ("\n", "\r", "\t"):
                cleaned_chars.append(ch)
            # Всё остальное (эмодзи, нестандартные символы) — пропускаем

    text = "".join(cleaned_chars)

    # Шаг 3: чистим артефакты — множественные пробелы/пустые строки
    text = re.sub(r" {2,}", " ", text)          # 2+ пробела → один
    text = re.sub(r"\n{3,}", "\n\n", text)      # 3+ переноса → два
    text = text.strip()

    # Шаг 4: обрезка по байтам cp1251, не ломая символы
    if not text:
        return ""
    ...
    # Обрезаем байты и декодируем обратно
    # cp1251 — однобайтовая кодировка, поэтому обрезка безопасна
    truncated = encoded[:max_bytes].decode("cp1251", errors="ignore")
    # Добавляем маркер обрезки, если влезает
    suffix = " [...]"
...
class AsyncICQEchoBot:
    ...
    async def _send_cli_families(self):
        families = [
            0x0022, 0x0001, 0x0001, 0x0004, 0x0013, 0x0004,
            0x0002, 0x0001, 0x0003, 0x0001, 0x0015, 0x0001,
            0x0004, 0x0001, 0x0006, 0x0001, 0x0009, 0x0001,
            0x000a, 0x0001, 0x000b, 0x0001,
        ]
        await self._send_snac(0x0001, 0x0017,
                              b"".join(struct.pack("!H", f) for f in families))

    async def _send_cli_setuserinfo(self):
        caps = [CAP_AIM_SERVERRELAY, CAP_DIRECT, CAP_UTF8]
        if self.xstatus_guid:
            caps.append(self.xstatus_guid)
        await self._send_snac(0x0002, 0x0004,
                              make_tlv(0x0005, b"".join(caps))
                              + make_tlv(0x0006, struct.pack("!I", self.status_flags)))
    ...
    async def _send_cli_setdcinfo(self):
        await self._send_snac(0x0001, 0x001E,
                              struct.pack("!IIIIHHHH", 0, 0, 0, 0, 0x000B, 0, 0, 0))
    ...
    async def _initialize_client(self):
        ...
        logging.info("Client fully initialized")
    ...
    async def _send_message(self, to_uin: str, text: str):
        """
        Отправляет сообщение. Перед отправкой:
        - санирует текст (убирает нестандартные символы/эмодзи)
        - обрезает до ICQ_MAX_MSG_BYTES байт
        Любые ошибки пойманы — бот не упадёт.
        """
        try:
            # ── Санирование и обрезка ──────────────────────────────────────
            # sanitize_for_icq чистит Unicode/эмодзи из нормального текста.
            # Но если upstream (command_handler) уже прогнал текст через encode с errors="replace",
            # эмодзи превратились в '?' до нашего вызова. Убиваем длинные серии '?' тоже.
            safe_text = sanitize_for_icq(text)
            # Серии из 2+ вопросительных знаков — артефакты замены эмодзи выше по стеку
            safe_text = re.sub(r'\?{2,}', '', safe_text)
            # Схлопываем пробелы/переносы которые могли остаться
            safe_text = re.sub(r' {2,}', ' ', safe_text)
            safe_text = re.sub(r'\n{3,}', '\n\n', safe_text).strip()
            if not safe_text:
            ...
            # sanitize_for_icq уже гарантирует cp1251-совместимость и длину.
            # errors="ignore" — последний рубеж на случай багов в sanitize.
            # errors="replace" запрещён: символ ? (0x3F) копится в пакете и сервер
            # может счесть пакет мусором и разорвать соединение.
            text_encoded = safe_text.encode("cp1251", errors="ignore")

            # Жёсткий потолок: режем если sanitize пропустила лишнее
            ...
            # Ошибки соединения пробрасываем выше — пусть run() их поймает и переподключится
            logging.error(f"Connection error while sending to {to_uin}: {e}")
            raise
        except Exception as e:
            # Всё остальное (баги пакетирования, кодировки и т.д.) — логируем и глотаем
    ...
    async def _process_message(self, sender_uin: str, message_text: str):
        ...
        try:
            ...
            # Соединение уже упало — _message_loop обнаружит это сам и запустит переподключение.
            # Пробрасывать не нужно: задача фоновая, и необработанное исключение даст
            # "Task exception was never retrieved" в лог.
    ...
    async def run(self):
        ...
        while not self._stop_requested:
            ...
            try:
                ...
                logging.info(f"ICQ bot ready (max_concurrent={self.semaphore._value})")
