...
WEATHER_CODES = {
    ...
    77: "Снежная крупа", 80: "Кратковременный дождь", 81: "Ливневый дождь",
    82: "Сильный ливень", 85: "Слабый снегопад", 86: "Сильный снегопад",
...
def get_user_city(user_id: str) -> Optional[str]:
    return load_user_cities().get(user_id)
...
def deg_to_wind(deg: float) -> str:
    return WIND_DIRECTIONS[round(deg / 45) % 8]
...
async def get_coordinates(city: str):
    ...
    for item in data:
        if item.get("class") == "place" and item.get("type") in place_types:
            ...
            city_name = address["city"] if address and "city" in address else item["display_name"].split(",")[0]
            return float(item["lat"]), float(item["lon"]), city_name

    i = data[0]
    address = i.get("address")
    city_name = address["city"] if address and "city" in address else i["display_name"].split(",")[0]
...
def build_data(city: str, data: dict) -> dict:
    ...
    return {
        ...
        "pressure": round(pressure * 0.750062) if pressure else None,
