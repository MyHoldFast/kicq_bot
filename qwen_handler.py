...
def remove_markdown(text: str) -> str:
    """Убирает Markdown-разметку, сохраняя читаемую структуру для ICQ."""

    # 1. Блоки кода — первыми, чтобы их содержимое не трогали другие замены
    ...
    # 3. Жирный курсив, жирный, курсив (порядок важен: сначала тройной)
    ...
    # 9. Не более двух пустых строк подряд
    text = re.sub(r'\n{3,}', '\n\n', text)
...
def emoji_to_simple(text: str) -> str:
    """Заменяет известные эмодзи на текстовые смайлики."""
    emoji_map = {
        '😊': ':)', '🙂': ':)', '😄': ':-D', '😃': ':-D', '😁': ':-D',
        '😆': ':-D', '😂': '^_^', '😉': ';)', '😍': ':*', '😘': ':*',
        '😋': ':P', '😛': ':P', '😜': ';P', '🤔': ':|', '😐': ':|',
...
def remove_unhandled_emoji(text: str) -> str:
    """Удаляет все эмодзи и декоративные Unicode-символы, не вошедшие в emoji_to_simple."""
    emoji_pattern = re.compile(
        "["
        "\U0001F300-\U0001F9FF"   # основные эмодзи (смайлы, животные, еда, флаги...)
        "\U0001FA00-\U0001FAFF"   # расширенные эмодзи (шахматы, бытовые предметы...)
        "\U00002600-\U000027BF"   # разные символы (☀☁✓★♠...)
        "\U0000FE00-\U0000FE0F"   # variation selectors (невидимые модификаторы эмодзи)
        "\U0001F1E0-\U0001F1FF"   # региональные индикаторы (флаги стран)
        "\U00002702-\U000027B0"   # дингбаты (✂✈✉...)
        "\U000024C2-\U0001F251"   # заключённые символы, карты, маджонг...
        "]+",
        flags=re.UNICODE
    )
...
def clean_for_win1251(text: str) -> str:
    """
    Финальная очистка для windows-1251:
